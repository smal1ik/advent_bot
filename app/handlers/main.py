...
async def cmd_message(message: types.Message, bot: Bot, command: Command):
    ...
    if member.status in ("member", "administrator", "creator"):
        await message.answer(cp.start_msg, reply_markup=kb.get_advent_btn(check_days=user.check_days))
    else:
...
async def check_sub(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if member.status in ("member", "administrator", "creator"):
        ...
        await callback.message.answer(cp.start_msg, reply_markup=kb.get_advent_btn(check_days=user.check_days))
    else:
